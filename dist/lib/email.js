...
exports.emailTemplates = {
    waitlistConfirmation: {
        subject: (planType) => `Welcome to ConvertViral ${planType} Waitlist! 🎉`,
        html: (data) => `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    leadMagnetWelcome: {
        subject: (magnetType) => `Your Free ${magnetType.charAt(0).toUpperCase() + magnetType.slice(1)} is Ready! 📚`,
        html: (data) => `
      <!DOCTYPE html>
      <html>
      <head>
...
async function sendWaitlistConfirmation({ email, name, planType, discountCode, referralCode }) {
    const emailService = createEmailService();
    const template = exports.emailTemplates.waitlistConfirmation;
...
async function sendLeadMagnetWelcome({ email, name, magnetType, downloadUrl }) {
    const emailService = createEmailService();
    const template = exports.emailTemplates.leadMagnetWelcome;
    await emailService.send({
        to: email,
...
async function sendLaunchNotification({ email, name, planType, discountCode }) {
    const emailService = createEmailService();
    const template = exports.emailTemplates.launchNotification;
    await emailService.send({
        to: email,
...
class EmailAutomation {
    static async scheduleWaitlistSequence({ email, name, planType, discountCode, referralCode }) {
    ...
    static async scheduleLeadMagnetSequence({ email, name, magnetType, downloadUrl }) {
