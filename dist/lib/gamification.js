...
async function getLeaderboard(limit = 10, timeframe = 'allTime', country) {
    try {
        // Try to get from cache first
        const cacheKey = `leaderboard:${timeframe}:${country || 'all'}:${limit}`;
        const cachedLeaderboard = await (0, redis_1.getCache)(cacheKey);
        ...
        await (0, redis_1.setCache)(cacheKey, leaderboard, 300);
        return leaderboard;
    }
    catch (error) {
...
async function getUserRank(userId, timeframe = 'allTime') {
    try {
        // Get user's points
        ...
        const usersWithMorePoints = await db_1.prisma.user.count({
            where: {
                points: {
                    gt: user.points,
                },
...
async function getUserLevel(userId) {
    try {
        ...
        let levelInfo = exports.LEVELS.BEGINNER;
...
async function awardPointsForConversion(userId, sourceFormat, targetFormat, batchId) {
    try {
        // Determine points based on format type
        let points = exports.POINTS.CONVERSION.DEFAULT;
        ...
        await db_1.prisma.conversion.update({
            where: {
        ...
        // Record daily points
        await db_1.prisma.dailyPoints.create({
            data: {
                userId,
                points: totalPoints,
                source: 'conversion'
    ...
    try {
        ...
        for (const badge of allBadges) {
            ...
            if (criteria.pdf_conversions && user.conversions) {
                const pdfConversions = user.conversions.filter((conv) => conv.sourceFormat.includes('pdf') || conv.targetFormat.includes('pdf')).length;
            ...
            if (criteria.shares) {
                ...
                const shareCount = await db_1.prisma.dailyPoints.count({
                    where: {
                        userId,
                        source: 'share'
                    }
...
async function awardPointsForSharing(userId, shareType) {
    try {
        let points = 0;
        switch (shareType) {
            case 'conversion':
...
async function awardPointsForReferral(userId, referredUserId) {
    try {
        const points = exports.POINTS.REFERRAL;
...
async function initializeDailyChallenges() {
    try {
        ...
        for (const challenge of defaultChallenges) {
            await db_1.prisma.challenge.upsert({
...
async function getDailyChallenges(userId) {
    try {
        ...
        const existingChallenges = await db_1.prisma.userChallenge.findMany({
            where: {
                userId,
                createdAt: {
                    gte: startOfDay,
        ...
        if (existingChallenges.length > 0) {
            ...
            return existingChallenges.map((uc) => ({
                id: uc.id,
                userId: uc.userId,
        ...
        // Get all challenges
        const allChallenges = await db_1.prisma.challenge.findMany();
        ...
        for (const challenge of selectedChallenges) {
            const userChallenge = await db_1.prisma.userChallenge.create({
                data: {
                    userId,
...
async function checkChallengeProgress(userId) {
    try {
        // Get today's date (start and end)
        const today = new Date();
        const startOfDay = new Date(today.getFullYear(), today.getMonth(), today.getDate());
        ...
        const userChallenges = await db_1.prisma.userChallenge.findMany({
            where: {
                userId,
                createdAt: {
                    gte: startOfDay,
        ...
        const userConversions = await db_1.prisma.conversion.findMany({
            where: {
                userId,
                createdAt: {
                    gte: startOfDay,
        ...
        const userShares = await db_1.prisma.dailyPoints.count({
            where: {
                userId,
                date: {
                    gte: startOfDay,
        ...
        const userReferrals = await db_1.prisma.dailyPoints.count({
            where: {
                userId,
                date: {
                    gte: startOfDay,
        ...
        for (const userChallenge of userChallenges) {
            ...
            if (criteria.new_format) {
                ...
                const previousFormats = await db_1.prisma.conversion.findMany({
                    where: {
                        userId,
                        createdAt: {
                            lt: startOfDay
            ...
            if (criteria.image_to_pdf) {
                const imageToPdfCount = userConversions.filter((conv) => conv.sourceFormat.startsWith('image/') && conv.targetFormat.includes('pdf')).length;
            ...
            if (criteria.video_conversions) {
                const videoCount = userConversions.filter((conv) => conv.sourceFormat.startsWith('video/') || conv.targetFormat.startsWith('video/')).length;
            ...
            if (criteria.audio_conversions) {
                const audioCount = userConversions.filter((conv) => conv.sourceFormat.startsWith('audio/') || conv.targetFormat.startsWith('audio/')).length;
            ...
            await db_1.prisma.userChallenge.update({
